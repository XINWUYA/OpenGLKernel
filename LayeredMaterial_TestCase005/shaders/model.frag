...
void main()
{
	vec3 OriginalColor = texture(u_DiffuseTexture, v2f_TextureCoord).rgb;
	float DiffuseAO = texture(u_AOTexture, v2f_TextureCoord).r;
...
//	vec3 AmbientColor = AmbientWeight * OriginalColor;
	...
	BRDFParameterInfo.Roughness = CalculateRoughnessWithCleaCoat(u_Roughness, u_ClearCoatRoughness, u_ClearCoatThickness);//CalculateRoughnessWithCleaCoat(texture(u_RoughnessTexture, v2f_TextureCoord).r/*u_Roughness*/, u_ClearCoatRoughness, u_ClearCoatThickness);
	BRDFParameterInfo.Metallic = u_Metallic;//texture(u_MetallicTexture, v2f_TextureCoord).r;//u_Metallic;

	float Distance = length(u_LightInfo.Position - v2f_FragPos);
	//Direct Light PBR
	vec3 ResultColor = DiffuseAO * u_LightInfo.Intensity * CalculateBRDF(BRDFParameterInfo) * u_LightInfo.Color / (Distance * Distance);

	float SpecularAO = CalculateSpecularAO(max(dot(BRDFParameterInfo.Normal, BRDFParameterInfo.ViewDir), 0.0f), DiffuseAO, BRDFParameterInfo.Roughness);
	//IBL Difuse
	...
	vec3 IBLDiffuseColor = DiffuseAO * OriginalColor * Kd * texture(u_IrradianceMap, Normal).rgb;
	...
	vec3 IBLSpecularColor = SpecularAO * PrefilteredEnvironmentColor * (Ks * BRDFTerm.x + BRDFTerm.y);
