...
void CGraphicsApp::run()
{
	while (!glfwWindowShouldClose(m_pGLFWWindow) && !glfwGetKey(m_pGLFWWindow, GLFW_KEY_ESCAPE))
	{
		...
		m_pShader->setVec3("Light.m_Position", CCamera::get_mutable_instance().getCameraPosition());
		m_pShader->setVec3("Light.m_Direction", CCamera::get_mutable_instance().getCameraFront());
		m_pShader->setFloat("Light.m_InnerCutOffAngle", glm::cos(glm::radians(12.5f)));
		m_pShader->setFloat("Light.m_OuterCutOffAngle", glm::cos(glm::radians(20.0f)));
...
void CGraphicsApp::__initShader()
{
	...
	m_pShader->addShader("../ShaderSources/LN008/CubeVertShader.glsl", VERTEX_SHADER);
	m_pShader->addShader("../ShaderSources/LN008/CubeFragShader.glsl", FRAGMENT_SHADER);
	m_pShader->createShaderProgram();

	m_pLightShader = new CShader();
	m_pLightShader->addShader("../ShaderSources/LN008/LightVertShader.glsl", VERTEX_SHADER);
	m_pLightShader->addShader("../ShaderSources/LN008/LightFragShader.glsl", FRAGMENT_SHADER);
