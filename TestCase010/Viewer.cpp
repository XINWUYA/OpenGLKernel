...
CViewer::CViewer(const std::string& WindowTitle, int vWindowWidth, int vWindowHeight) : CGLScreen(WindowTitle, vWindowWidth, vWindowHeight, false, 8), m_WindowWidth(vWindowWidth), m_WindowHeight(vWindowHeight)
{
	...
	m_LightMovingDir = glm::normalize(glm::vec3(gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f),
												gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f),
												gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f)));
}
...
void CViewer::drawContentsV()
{
	...
	m_pGLShader->setIntUniform("u_IsUsePBR", m_IsUsePBR);
	m_pModel->draw();
...
void CViewer::processKeyCallbackEventV(int vKey, int vScancode, int vAction, int vMods)
{
	...
	if (vKey == GLFW_KEY_P && vAction == GLFW_PRESS)
		m_IsDynamicLight = !m_IsDynamicLight;
}
...
//***********************************************************************************************
//Function:
void CViewer::__updateLightPos()
{
	glm::vec3 NextPos = glm::vec3(m_LightPosX, m_LightPosY, m_LightPosZ) + m_LightMovingStepLength * m_LightMovingDir;

	if (std::abs(m_LightPosX) >= 5.0f || std::abs(m_LightPosY) >= 5.0f || std::abs(m_LightPosZ) >= 5.0f)
	{
		m_LightMovingDir = glm::normalize(glm::vec3(gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f),
													gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f),
													gl_kernel::generateRandomValue<float>(m_RandomEngine, -1.0f, 1.0f)));
		NextPos = glm::vec3(m_LightPosX, m_LightPosY, m_LightPosZ) + m_LightMovingStepLength * m_LightMovingDir;
	}

	m_LightPosX = NextPos.x;
	m_LightPosY = NextPos.y;
	m_LightPosZ = NextPos.z;
}
