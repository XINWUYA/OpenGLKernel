...
void CViewer::drawContentsV()
{
	...
	if (m_SelectedLabel == "DoubleLayer")
		m_IsUseDoubleLayer = true;
	else
		m_IsUseDoubleLayer = false;
	...
	m_pGLShader->setFloatUniform("u_Roughness", m_ModelRoughness);
	m_pGLShader->setFloatUniform("u_Metallic", m_ModelMetallic);
	...
	m_pGLShader->setIntUniform("u_IsUseDoubleLayer", m_IsUseDoubleLayer);
	//m_pModel->draw();
...
void CViewer::__setGUIComponents()
{
	...
	if (m_IsUseDoubleLayer)
	{
		m_pAssistGUI->text("UpperLayerParameter:");
