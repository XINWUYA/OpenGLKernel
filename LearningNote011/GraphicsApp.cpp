...
CGraphicsApp::CGraphicsApp(int vWindowWidth, int vWindowHeight, std::string& vWindowName) : m_pGLFWWindow(nullptr), m_pShader(nullptr), m_pLightShader(nullptr), m_pStencilShader(nullptr), m_pTexture(nullptr), m_pTexture1(nullptr), m_pModel(nullptr), m_IsStencilTestOpened(false)
{
...
//**********************************************************************************
//FUNCTION:
void CGraphicsApp::openStencilTest()
{
	glEnable(GL_STENCIL_TEST);
	glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
	m_IsStencilTestOpened = true;
}
...
void CGraphicsApp::run()
{
	while (!glfwWindowShouldClose(m_pGLFWWindow) && !glfwGetKey(m_pGLFWWindow, GLFW_KEY_ESCAPE))
	{
		...
		m_pModel->drawModel(m_pShader, 0);
		...
		if (m_IsStencilTestOpened)
		{
			...
			m_pStencilShader->useShaderProgram();
			m_pStencilShader->setMat4("uProjection", Projection);
			m_pStencilShader->setMat4("uView", View);
			glm::mat4 StencilModel;
			StencilModel = glm::translate(StencilModel, glm::vec3(0.0f, -1.79f, 0.0f));
			StencilModel = glm::scale(StencilModel, glm::vec3(0.205f));
			m_pStencilShader->setMat4("uModel", StencilModel);
			m_pModel->drawModel(m_pStencilShader, 0);
			glStencilMask(0xFF);
			glEnable(GL_DEPTH_TEST);
		}
...
void CGraphicsApp::__initShader()
{
	...
	m_pShader->addShader("../ShaderSources/LN011/NanosuitVertShader.glsl", VERTEX_SHADER);
	m_pShader->addShader("../ShaderSources/LN011/NanosuitFragShader.glsl", FRAGMENT_SHADER);
	m_pShader->createShaderProgram();

	m_pLightShader = new CShader();
	m_pLightShader->addShader("../ShaderSources/LN011/LightVertShader.glsl", VERTEX_SHADER);
	m_pLightShader->addShader("../ShaderSources/LN011/LightFragShader.glsl", FRAGMENT_SHADER);
	m_pLightShader->createShaderProgram();

	m_pStencilShader = new CShader();
	m_pStencilShader->addShader("../ShaderSources/LN011/StencilVertShader.glsl", VERTEX_SHADER);
	m_pStencilShader->addShader("../ShaderSources/LN011/StencilFragShader.glsl", FRAGMENT_SHADER);
	m_pStencilShader->createShaderProgram();
}
