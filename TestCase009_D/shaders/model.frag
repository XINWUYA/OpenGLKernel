...
uniform float u_Roughness = 1.0f;
...
//	vec3 AmbientColor = AmbientWeight * OriginalColor;
	...
	//IBL Difuse
	vec3 F0 = mix(BRDFParameterInfo.F0, OriginalColor, u_Metallic);
	vec3 Ks = FresnelSchlickRoughness(max(dot(Normal, BRDFParameterInfo.ViewDir), 0.0f), F0, u_Roughness);
	vec3 Kd = vec3(1.0f) - Ks;
	Kd *= 1.0f - u_Metallic;
	...
	vec3 PrefilteredEnvironmentColor = textureLod(u_EnvironmentMap, RelfectDir, u_Roughness * MAX_REFLECTION_LOD).rgb;
	vec2 BRDFTerm = texture(u_BRDFTexture, vec2(max(dot(Normal, BRDFParameterInfo.ViewDir), 0.0f), u_Roughness)).rg;
