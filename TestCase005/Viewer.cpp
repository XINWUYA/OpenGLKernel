...
CViewer::CViewer(const std::string& WindowTitle, int vWindowWidth, int vWindowHeight) : CGLScreen(WindowTitle, vWindowWidth, vWindowHeight, false, true), m_WindowWidth(vWindowWidth), m_WindowHeight(vWindowHeight)
{
	m_LastMouseX = static_cast<float>(vWindowWidth) / 2.0;
	m_LastMouseY = static_cast<float>(vWindowHeight) / 2.0;

	m_pCamera = std::make_shared<CCamera>(glm::vec3(0.0f, -1.0f, 3.0f));
	/*glm::mat4 CameraRotate = glm::mat4(1.0f);
	CameraRotate = glm::rotate(CameraRotate, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	m_pCamera->rotate(CameraRotate);*/
...
void CViewer::__processInput()
{
	...
	if (glfwGetKey(fetchGLFWWindow(), GLFW_KEY_C) == GLFW_PRESS)
		m_pCamera->printCurrentCameraFront();
}