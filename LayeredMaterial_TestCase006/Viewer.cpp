...
CViewer::CViewer(const std::string& WindowTitle, int vWindowWidth, int vWindowHeight) : CGLScreen(WindowTitle, vWindowWidth, vWindowHeight, false, 8), m_WindowWidth(vWindowWidth), m_WindowHeight(vWindowHeight)
{
	...
	setWindowIcon("../TextureSources/touxiang.jpg");
	...
	m_pModel = std::make_shared<CGLModel>("../ModelSources/substance_sphere/substance_sphere.obj");
	m_pModel->init(*m_pDrawModelShader);

	gl_kernel::STexture Texture;
	Texture.m_IsFLipVertically = GL_FALSE;
	Texture.m_Type4WrapS = Texture.m_Type4WrapT = GL_CLAMP_TO_EDGE;
	m_pDiffuseTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/metal/albedo.png", Texture);
	m_pNormalTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/metal/normal.png", Texture);
	m_pMetallicTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/metal/metallic.png", Texture);
	m_pRoughnessTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/metal/roughness.png", Texture);
	m_pAOTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/ao.png", Texture);
	m_pIridescenceThickTexture = std::make_shared<CGLTexture>("../ModelSources/substance_sphere/masks/edge_blur.png", Texture);
	...
	m_pDrawModelShader->bind();
	m_pDiffuseTexture->bind(m_pDiffuseTexture->getTextureID() + 5);//if a model added, the id shoule + 5.
	m_pDrawModelShader->setIntUniform("u_DiffuseTexture", m_pDiffuseTexture->getTextureID() + 5);
	m_pNormalTexture->bind(m_pNormalTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_NormalTexture", m_pNormalTexture->getTextureID() + 5);
	m_pMetallicTexture->bind(m_pMetallicTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_MetallicTexture", m_pMetallicTexture->getTextureID() + 5);
	m_pRoughnessTexture->bind(m_pRoughnessTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_RoughnessTexture", m_pRoughnessTexture->getTextureID() + 5);
	m_pAOTexture->bind(m_pAOTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_AOTexture", m_pAOTexture->getTextureID() + 5);
	m_pIridescenceThickTexture->bind(m_pIridescenceThickTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_IridescenceThickTexture", m_pIridescenceThickTexture->getTextureID() + 5);
	m_pIrradianceMapTexture->bind(m_pIrradianceMapTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_IrradianceMap", m_pIrradianceMapTexture->getTextureID() + 5);
	m_pEnvironmentMapTexture->bind(m_pEnvironmentMapTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_EnvironmentMap", m_pEnvironmentMapTexture->getTextureID() + 5);
	m_pBRDFTexture->bind(m_pBRDFTexture->getTextureID() + 5);
	m_pDrawModelShader->setIntUniform("u_BRDFTexture", m_pBRDFTexture->getTextureID() + 5);
...
void CViewer::drawContentsV()
{
	...
	Model = glm::rotate(Model, m_RotateSpeed * (float)glfwGetTime()/*glm::radians(45.0f)*/, glm::vec3(0.0f, 1.0f, 0.0f));
	...
	m_pDrawModelShader->setFloatUniform("u_Reflectance", m_Reflectance);
	m_pDrawModelShader->setFloatUniform("u_ClearCoatRoughness", m_ClearCoatRoughness);
	m_pDrawModelShader->setFloatUniform("u_ClearCoatThickness", m_ClearCoatThickness);
	m_pDrawModelShader->setFloatUniform("u_ThinFilmIridescene", m_ThinFilmIridescene);
	m_pDrawModelShader->setFloatUniform("u_ThinFilmThickness", m_ThinFilmThickness);
	m_pDrawModelShader->setFloatUniform("u_ThinFilmIOR", m_ThinFilmIOR);
	m_pDrawModelShader->setFloatUniform("u_ThinFilmExtinctionK", m_ThinFilmExtinctionK);
...
void CViewer::__setGUIComponents()
{
	...
	m_pAssistGUI->text("ClearCoat Layer Info:");
	m_pAssistGUI->sliderFloat("Reflectance", m_Reflectance, 0.0f, 1.0f);
	m_pAssistGUI->sliderFloat("ClearCoatRoughness", m_ClearCoatRoughness, 0.0f, 1.0f);
	m_pAssistGUI->sliderFloat("ClearCoatThickness", m_ClearCoatThickness, 0.0f, 1.0f);

	m_pAssistGUI->text("Thin Film Layer Info:");
	m_pAssistGUI->sliderFloat("ThinFilmIridescene", m_ThinFilmIridescene, 0.0f, 1.0f);
	m_pAssistGUI->sliderFloat("ThinFilmThickness", m_ThinFilmThickness, 0.0f, 1.0f);
	m_pAssistGUI->sliderFloat("ThinFilmIOR", m_ThinFilmIOR, 1.0f, 5.0f);
	m_pAssistGUI->sliderFloat("ThinFilmExtinctionK", m_ThinFilmExtinctionK, 0.0f, 5.0f);
}
